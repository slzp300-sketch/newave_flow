...
// localStorage 키 (토요일 교사회의용)
function getSatKey() {
  const { weekNum } = getTTSWeekRange()
  const year = getNow().getFullYear()
  return `sat_meeting_${year}_w${weekNum}`
}

function buildSatState() { return { status: null, reason: '' } }
...
// ── 기도모임 제출 완료 카드 ─────────────────
function PrayerSubmittedCard({ vote, micLabel, onEdit, isMicTurn }) {
